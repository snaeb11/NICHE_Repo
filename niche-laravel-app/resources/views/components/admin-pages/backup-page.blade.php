...
@if (auth()->user()->hasPermission('view-backup'))
    <div id="confirm-input-popup" class="fixed inset-0 bg-black/50 items-center justify-center hidden z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-[90%] max-w-md space-y-4 text-center">
            ...
            <input type="text" id="confirm-name-input" class="border border-gray-300 rounded w-full px-3 py-2 mt-2"
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2 pt-2">
                ...
                <button id="confirm-submit-btn"
                    class="px-4 py-2 bg-[#28C90E] text-white rounded hover:brightness-110 text-sm">Confirm
                    Restore</button>
            </div>
        </div>
    </div>
        ...
        <div class="flex justify-center items-center px-4">
            <div
                ...
                @if (auth()->user()->hasPermission('allow-restore'))
                    <div
                        class="flex flex-col items-center justify-center rounded border-dashed border-[#575757] border p-4 w-full">
                        <span class="text-[#575757] text-sm py-1 font-semibold">Choose a file or drag & drop it
                            here.</span>
            ...
            @if (auth()->user()->hasPermission('allow-restore'))
                <form id="restore-form" action="{{ route('admin.backup.restore') }}" method="post"
                    ...
                    <button type="button" id="restore-btn"
                        class="px-4 py-2 bg-gradient-to-r from-[#28C90E] to-[#1CA305] text-white rounded w-full hover:brightness-110 cursor-pointer">Restore</button>
                </form>
            @else
