...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['undergraduate', 'graduate', 'advisers']));
...
foreach (array_filter((['undergraduate', 'graduate', 'advisers']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<main id="inventory-table" class="ml-[4vw] hidden p-8 transition-all duration-300 ease-in-out group-hover:ml-[18vw]">
        ...
        <div class="w-full">
            <div class="mb-5 flex w-full items-center justify-between">
                ...
                <div class="flex flex-wrap justify-end gap-1 sm:gap-2">
                    <!-- Buttons -->
                    <?php if(auth()->user() && auth()->user()->hasPermission('add-inventory')): ?>
                    ...
                    <?php if(auth()->user() && auth()->user()->hasPermission('import-inventory')): ?>
                    ...
                    <?php if(auth()->user() && auth()->user()->hasPermission('export-inventory')): ?>
                ...
                <div class="flex flex-wrap justify-end gap-2 sm:gap-4">
                    ...
                    <div class="relative">
                        <select name="inv-dd-program"
                            ...
                            <?php if($undergraduate->isNotEmpty()): ?>
    ...
    <?php if(auth()->user() && auth()->user()->hasPermission('view-inventory')): ?>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-[#fdfdfd]">
                    <tr>
                        ...
                        <?php if(auth()->user() && auth()->user()->hasPermission('edit-inventory')): ?>
                            <th class="cursor-pointer px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
                                data-column="0" data-order="asc" onclick="sortTable(this)">
                                Actions
                            </th>
...
<main id="add-inventory-page"
    ...
    <form id="add-inventory-form" method="POST" action="<?php echo e(route('inventory.store')); ?>"
                    ...
                    <div class="flex flex-col space-y-1">
                        ...
                        <div class="relative">
                            <select name="adviser" id="adviser-select"
                                ...
                                <?php if($advisers->isNotEmpty()): ?>
                                    <?php $__currentLoopData = $advisers->groupBy('program.name'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $programName => $programAdvisers): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <optgroup label="<?php echo e($programName); ?>">
                                            <?php $__currentLoopData = $programAdvisers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $adviser): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($adviser->name); ?>"
                                                    data-program-id="<?php echo e($adviser->program_id); ?>"><?php echo e($adviser->name); ?>
                    ...
                    <div class="flex flex-col space-y-1">
                        ...
                        <div class="relative">
                            <input type="number" name="academic_year" id="academic-year-input" min="1990"
                                max="<?php echo e(date('Y')); ?>" value="<?php echo e(date('Y')); ?>"
                    ...
                    <div class="flex flex-col space-y-1">
                        ...
                        <div class="relative">
                            <input type="number" name="academic_year" id="edit-academic-year-input" min="1990"
                                max="<?php echo e(date('Y')); ?>" placeholder="Enter year (e.g., 2024)"
                                class="no-spinner min-h-[44px] w-full rounded-xl border-2 border-gray-200 bg-white px-4 py-3 font-medium text-[#575757] placeholder-gray-400 shadow-sm transition-all duration-300 focus:border-[#CE6767] focus:shadow-md focus:outline-none focus:ring-2 focus:ring-[#CE6767]/20 sm:min-h-[48px]">
                            <div class="absolute right-4 top-1/2 -translate-y-1/2 transform text-[#575757]">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<?php if(session('success')): ?>
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            xSubTextSuc.textContent = <?php echo json_encode(session('success'), 15, 512) ?>;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (addForm) {
            ...
            async function checkDuplicateTitle(rawTitle) {
                ...
                try {
                    const url = `<?php echo e(route('inventory.check-duplicate-title')); ?>?title=` +
...
<?php /**PATH /home/vunc/Documents/HR_Repo/HR_Repo/niche-laravel-app/resources/views/components/admin-pages/inventory-page.blade.php ENDPATH**/ ?>