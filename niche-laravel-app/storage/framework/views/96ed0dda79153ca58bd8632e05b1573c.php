<div id="downloadable-files-popup-one"
    ...
    <div class="relative max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div class="max-h-[calc(90vh-120px)] overflow-y-auto p-6">
            <!-- Loading State -->
            <div id="rndd-forms-loading" class="py-8 text-center">
            ...
            <div id="rndd-forms-grid" class="grid hidden grid-cols-1 gap-4 md:grid-cols-2">
                <!-- Forms will be loaded here dynamically -->
            </div>

            <!-- Empty State -->
            <div id="rndd-forms-empty" class="hidden py-8 text-center">
                <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <h3 class="mt-2 text-sm font-medium text-gray-900">No forms available</h3>
                <p class="mt-1 text-sm text-gray-500">No R&DD forms are currently available.</p>
            </div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const popup = document.getElementById('downloadable-files-popup-one');
        const loadingDiv = document.getElementById('rndd-forms-loading');
        const gridDiv = document.getElementById('rndd-forms-grid');
        const emptyDiv = document.getElementById('rndd-forms-empty');
        ...
        async function loadRnddForms() {
            try {
                loadingDiv.classList.remove('hidden');
                gridDiv.classList.add('hidden');
                emptyDiv.classList.add('hidden');

                const response = await fetch('/downloadable-forms/rndd_forms');
                if (!response.ok) {
                ...
                console.error('Error loading R&DD forms:', error);
                loadingDiv.classList.add('hidden');
                emptyDiv.classList.remove('hidden');
            }
        }
...
<?php /**PATH /home/vunc/Documents/HR_Repo/HR_Repo/niche-laravel-app/resources/views/components/popups/downloadable-files-one.blade.php ENDPATH**/ ?>